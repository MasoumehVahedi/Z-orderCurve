...
class IntervalCracking:
    ...
    def searchAndCrack(self, query_interval, query, node):
        ...
        """
        xmin_query, ymin_query, xmax_query, ymax_query = query
        query_results = []
        ...
        if len(node.entries) <= self.max_entries:
            query_results.extend(
                [entry.data for entry in node.entries if not (entry.data[2] < xmin_query or entry.data[0] > xmax_query or  entry.data[3] < ymin_query or  entry.data[1] > ymax_query)])
            return query_results
